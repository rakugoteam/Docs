...
const registerOnScrollEvent = (function () {
  ...
  const menuHeightOffset_diff =
    menuHeightOffset_default - menuHeightOffset_fixed;
    ...
    if (mediaQuery.matches) {
      ...
      const handleMainScroll = (currentScroll) => {
        if (currentScroll >= scrollTopPixels) {
          ...
          $menu.css(
            "margin-top",
            `${menuTopMargin + (scrollTopPixels - currentScroll)}px`
          );

          if (currentScroll > 0) {
            const scrolledPercent =
              (scrollTopPixels - currentScroll) / scrollTopPixels;
            const offsetValue =
              menuHeightOffset_fixed + menuHeightOffset_diff * scrolledPercent;
            $menu.css("max-height", `calc(100% - ${offsetValue}px)`);
          } else {
            $menu.css(
              "max-height",
              `calc(100% - ${menuHeightOffset_default}px)`
            );
          }
        }
      };
      ...
      const handleSidebarScroll = () => {
        ...
        if (menuScrollBottom < ethicalOffsetBottom) {
          ...
          $ethical.css(
            "margin-top",
            `-${ethicalOffsetBottom - menuScrollBottom}px`
          );
        } else {
...
$(document).ready(() => {
  ...
  if (inDev) {
    ...
    document.querySelector('div[itemprop="articleBody"]').insertAdjacentHTML(
      "afterbegin",
      `
      <div class="admonition attention">
        ...
        <p>
          ...
          or compatible with Rakugo 3.1.x.
        </p>
        <p class="last">
